            ...
            for shape in shapes:
                ...
                if shape.shape_type == MSO_SHAPE_TYPE.PICTURE:
                    try:
                        ...
                        print(f"[convert] WARNING: failed to extract PPTX image on slide {i+1}")
            md_slides.append("\n\n".join(slide_content))
    ...
    try:
        ...
        for uri in uris:
            ...
            asset = _save_image_asset(
                ...
                caption=f"{IMG_ALT_PREFIX} - image {actual_idx}",
            )
            images.append(asset)
    ...
    try:
        ...
        if ftype == "pdf":
            ...
            for uri in uris:
                decoded = _data_uri_to_image(uri)
                if not decoded:
                    print(f"[convert] WARNING: failed to decode data URI for HTML image, skipping")
                    continue
                html_img_idx += 1
                im, _mime = decoded
                asset = _save_image_asset(
                    ...
                    image_index=html_img_idx,
                    caption=f"{IMG_ALT_PREFIX} - image {html_img_idx}",
                )
                images.append(asset)
